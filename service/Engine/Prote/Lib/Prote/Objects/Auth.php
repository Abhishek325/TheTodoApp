...
class Auth{
	...
	public function login($uname,$pwd,$auth_token){
		...
		$this->make_salt($uname,$pwd);
		$pwd=$this->generate_hash($pwd);
		if($id=$this->Service->Prote()->DBI()->user()->common()->verify($uname,$pwd)){	
			$_SESSION['id']=$id;
	...
	public function login_debug(){
		...
		$pwd=$this->generate_hash($pwd);
		$status=$this->Service->Prote()->DBI()->user()->data()->verify($email,$pwd,$auth_token);
		var_dump($status);
		exit();
	}
	...
	public function generate_reset_password_token($email,$auth_token){
		...
		$token=$this->generate_hash($email."#Cr0WDPULSE");
		...
		$mail->Subject = 'Password Reset- #CrowdPulse';
		//Read an HTML message body from an external file, convert referenced images to embedded,
		//convert HTML into a basic plain-text alternative body
		$mail->IsHTML(true);
		$reset_url='http://crowdpulse.com/password-reset-verify/'.$email.'/'.$token;
	...
	public function verify_reset_password_token($email,$token){
		...
		//THIS EXPOSES THE RESET KEY. CAN BE REUSED IF LEAKED OR SESSION HIJACKED.
			if($token==$this->generate_hash($email."#Cr0WDPULSE")){
				return 1;
			}
	}
